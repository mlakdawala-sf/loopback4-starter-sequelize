...
describe('User Controller', async () => {
  ...
  async function timeout(milis: number) {
  ...
  async function postUser(token: string) {
    await roleRepository.createAll([
      {
        id: 1,
        name: 'super_admin',
        roleKey: RoleType.SuperAdmin as any,
        // permissions: [
        //   'canLoginToIPS',
        //   'ViewOwnUser',
        //   'ViewAnyUser',
    ...
    await tenantRepository.create({
      id: 1,
      name: 'application',
      type: TenantType.APPLICATION as any,
    });
  ...
  async function deleteMockData() {
    await userRepo.deleteAllHard({truncate: true});
    await roleRepository.deleteAllHard({truncate: true});
    await tenantRepository.deleteAllHard({truncate: true});
  }
